...
- ✅ 设计树完整（每支走到叶子）· 5 个原型验证 · 全功能运行时 · M:N 并行 · **C 原生编译（zig cc/gcc）** · 双后端一致性（块/enum/match/数组/class 树/ref 字段通知/ref·move 参数/error/协作式并发）
- ⏳ 未启动：C 后端 M:N 多线程（当前 Fiber M=1 协作式）、带参 spawn、真竞争检测、类型标签版本字段
