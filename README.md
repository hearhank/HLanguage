...
| 测试 | `cargo test --workspace` **458 项全绿** |
| 示例回归 | 解释模式 **125/136 通过**（11 项失败属第三块 E1/E2 特性） |
| 原生交叉验证 | 编译模式 **52 项 mismatch**（未实现原生内建/方法，运行时以 `error.*` 响亮中止） |
| CI | 每次 push/PR 运行完整示例套件回归门（`tag1/scripts/check-examples.sh`） |
| 原生编译依赖 | 外部 `zig cc`（`hc build` / `hc test --mode=compile` 需要，缺失时回退字节码产物） |

> **注意**：`tag1/` 为**垂直切片**实现 —— 全部 7 个里程碑的核心功能已打通，非全量交付；余量（如 LLVM 原生仅内建子集、字节码 VM 复用 IR 语义）按模块登记于 `docs/SPEC/07-bootstrap-plan.md` 第八节与 `tag1/README.md`「已知取舍」。
...
├── docs/        # 设计文档：SPEC（01–07）、ADR 决策记录、review 裁定
├── examples/    # 示例套件（hello / 错误集 / struct / 跨包 / 网络 / 工具等 85+ 例）
...
实现计划分**三块**（`docs/SPEC/07-bootstrap-plan.md`），前两块构成**第一部分「最小功能集」**（`tag1` 已实现），第三块为**扩展与自举**。
...
| LLVM 原生 | `hc build <file.hc>` | 子集（已实现内建子集，52 mismatch 属此） |

### ⏳ 计划 —— 第三块 · 扩展与自举（E1–E7）

| 里程碑 | 内容 | 状态 |
|---|---|---|
| E1 元编程 | 脚本生成（`script` 块）、comptime 完整（类型即值）、泛型完整 | ⏳ 计划 |
| E2 并发与异步 | 线程 / 异步 / 四模式 / Send·Sync 静态标记 | ⏳ 计划 |
| E3 标准库扩展 | 四大支柱完整（含 UDP / HTTP / IPC / FFI 等） | ⏳ 计划 |
| E4 系统编程 | 系统编程特性（K1–K11） | ⏳ 计划 |
...
| T5（E1–E2 后） | 元编程 + 并发完整 | ⏳ 计划 |
| T6（E3–E5 后） | 标准库 + 工具链完整 | ⏳ 计划 |
| T7（E7 后） | 自举闭环（用 H 编译 H） | ⏳ 计划 |
| T8（E6 + 冻结） | 1.0 冻结 | ⏳ 计划 |

### 里程碑映射（`02-milestones.md` M0–M10 对照）

三块计划与 1.0 里程碑（M0–M10）的映射关系：第一块语言系统 ≈ 02 的 M1–M6；第二块最小外围 ≈ 02 的 M7–M8 主体；第三块承接脚本生成（M3）、并发（M5）、系统编程缺口、自举（M9）与 1.x 项。**1.0 的定义**（沿用 Zig 官方前置清单）：语言冻结 → 规范初稿 → 官方包管理器 → 标准库四大支柱 API 稳定 → 完整发布周期无破坏性变更。
...
| [`docs/SPEC/02-milestones.md`](docs/SPEC/02-milestones.md) | 阶段里程碑与验收标准（M0–M10） |
