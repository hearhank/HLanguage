...
> **2026-08-23**：E1 元编程、E2 并发与异步、E3 标准库扩展、E4 系统编程已全部落地；E5 工具链扩展大部分落地，B7 质量工具完整（LSP 跳转定义/hover/补全/格式化/lint 集）已完成；E6 部分落地（switch 守卫 + 开放问题裁决）；**四模式容器 Send·Sync 豁免已修复，hc test 147/148 全绿**；E7 计划中。完成文档整理：功能清单已补充16项特性，按18个分类、17个领域完整展示。vi
...
| E6 语言扩展 | 惰性迭代、switch 守卫、开放问题裁决、吃狗粮反馈 | 🟡 部分落地：switch 守卫已实施（模式+if 守卫+穷举检查）；开放问题裁决已定案（ADR-0016/0017）；惰性迭代真实现 1.x；吃狗粮反馈待自举阶段 |
