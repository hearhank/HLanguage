...
- **备注**：`hc pkg add`（本地依赖）已落地；build.zon 指纹 / 供应链校验未实现；注册中心治理（冲突 / 审计 / 失联）1.x
...
- **第三阶段活动项**：A 8（4 🔴 / 4 ⏳）+ B 7（4 🔴 / 2 🟡 / 1 ⏳）+ C 8（4 🔴 / 1 🟡 / 3 ⏳）+ D 2（1 🔴 / 1 🟡）+ E 4（4 ⏳）
- **第三阶段立即实施候选（🔴 且 1.x 无关）**：A1 ffi + B5 `hc cc`（联动）、B1 lint、B2 lsp 整合、B3 注册中心、C1 Table 多索引、C3 惰性迭代/switch 守卫/Send-Sync、C5 泛型嵌套、C7 原生 ABI、D1 并发测试、B6 启动时间、A8 端到端示例
