...
- **第三阶段活动项**：A 8（4 🔴 / 4 ⏳）+ B 7（4 🔴 / 2 🟡 / 1 ⏳）+ C 8（2 🔴 / 3 🟡 / 3 ⏳）+ D 2（1 🔴 / 1 🟡）+ E 4（4 ⏳）
- **第三阶段立即实施候选（🔴/🟡 且 1.x 无关）**：A1 ffi + B5 `hc cc`（联动）、B1 lint、B2 lsp 整合、B3 注册中心、**C1 Table 多索引**（设计已定案）、**C2 开放问题裁决**（设计已定案，ADR-0016）、**C3 惰性迭代/switch 守卫/Send-Sync**（设计已定案，ADR-0017）、C5 泛型嵌套、C7 原生 ABI、D1 并发测试、B6 启动时间、A8 端到端示例
