...
> **阶段定义（2026-08-22）**：第三阶段 = 标准库扩展 + 前两阶段未实现功能；自举 = 第四阶段；1.x 延迟项附录登记。
>
...
- **备注**：`iter()`/`filter()/map()` 立即求值链已落地；惰性 / 组合子迭代器 1.x
...
### C3｜惰性迭代、switch 守卫、Send/Sync 静态标记（编译期诊断）｜🔴
- **出处**：`10-part3-execution.md` 组 J2；`07-bootstrap-plan.md` E6.1
- **落点**：语义层（`semantic.rs`）+ 迭代契约（`CONTEXT.md` IIterable）
- **备注**：J2 三项；Send/Sync 与 C2 #6 联动——**语法形态已定案（2026-08-22，ADR-0016 #6）**：编译期接口（显式标注 + 可推导性验证），详细编译期诊断语义（哪些模式报错、如何推导）待本条目细化
...
- **备注**：`Vec<List<i32>>` 内建泛型外层嵌套仍退化裸名 `Vec`；`?` 自引用外无限大小类型语言层非法未处理
...
| 编号 | 功能 | 状态 | 出处 |
|---|---|---|---|
| Z1 | K2 H 版 parser / AST（token → 声明树）+ 对照测试 | 🔴 | `10-part3-execution.md` 组 K |
...
> `stage1/` 现状：`lexer.hc`（K1 H 版 lexer）已完成（git `12e8406`，自身源码 6621 token 零 diff + 对照语料全绿）；parser / 语义 / 后端未动工。

## 附录二：1.x 延迟项汇总

见上各条目（A2/A3/A4/A5/A6/A7、B4、C4、C6、E1/E2/E3/E4）。

---

## 统计

- **第三阶段活动项**：A 8（4 🔴 / 4 ⏳）+ B 7（4 🔴 / 2 🟡 / 1 ⏳）+ C 8（3 🔴 / 2 🟡 / 3 ⏳）+ D 2（1 🔴 / 1 🟡）+ E 4（4 ⏳）
- **第三阶段立即实施候选（🔴/🟡 且 1.x 无关）**：A1 ffi + B5 `hc cc`（联动）、B1 lint、B2 lsp 整合、B3 注册中心、**C1 Table 多索引**（设计已定案）、**C2 开放问题裁决**（设计已定案，ADR-0016）、C3 惰性迭代/switch 守卫/Send-Sync、C5 泛型嵌套、C7 原生 ABI、D1 并发测试、B6 启动时间、A8 端到端示例
