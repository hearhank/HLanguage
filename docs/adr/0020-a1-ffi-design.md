...
2. **`box(c_ptr, alloc)` = 复制进托管堆**：分配托管内存 + 按类型大小/布局复制 C 数据 → 返回 `o *mut T` **参与登记**（所有权归 H，悬垂检测适用）；与既有 `box` 语义一致（08-mem-allocator G3：box = 分配 + 值写入堆 + 带所有权指针）；「包裹外部指针」不给所有权、悬垂检测仍不适用，不采用
