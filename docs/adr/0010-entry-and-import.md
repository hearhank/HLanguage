...
1. **入口改为 `fn main(args: o Vec<String>) !void`**——main 不再接收 io 参数；`args` 由运行时注入（0 号 = 程序名，后续为命令行参数）；**命令行参数仅经入口注入，`io.args()` 取消（2026-08-17 F1 定案）**。io/alloc 作为**标准库模块与预导入环境**提供，不再经入口注入
