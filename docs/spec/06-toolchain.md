...
- 元组/切片（ADR 0007/0008）：**两后端已实现**（parser/checker/evaluator/cgen 同步落地）——
  ...
  - **嵌套元组字节化**：元组可作 struct 字段/数组元素（`h_tb_`/`h_jrev_` 递归嵌套，互相引用前向声明）——`Bag{pair: (1,2)}` 字节化/往返与求值器逐字节一致；
  - **切片 clone 复合元素**：递归块深拷贝 helper（`h_arr_clone_`/`h_tup_clone_`/`h_struct_clone_`，Str strdup，数组/元组/struct 任意嵌套）——`s.clone()` 对任意块元素深拷贝独立；
  - 数组类型 `[T]` 出现即注册切片候选（`T_Slice`），`arr[a..b]` 无需显式 `[]T` 声明即可生成。
