...
| D4 | ✅ comptime_int 常量折叠；✅ comptime_float 惰性宽度；✅ `anytype` 完整语义（调用点具体化）；⏳ `comptime` 值函数 | 语义测试绿 | D2 | 1.5h |
