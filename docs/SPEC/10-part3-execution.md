...
| D4 | ✅ comptime_int 常量折叠；⏳ comptime_float / `anytype` 完整语义 / `comptime` 值函数（惰性宽度、编译期常量折叠） | 语义测试绿 | D2 | 1.5h |
