...
- **原生 ABI（Phase 8 设计定案 2026-08-22，ADR-0019）**：函数值 = **胖闭包对象** `{ fn_ptr, env_ptr }`（堆上分配），`%Value` 载荷存其指针（新增闭包 tag，与 `T_PTR` 区分）；`FnRef` = 指向 LLVM 函数符号的地址（间接调用 = `call` 该地址）；**调用复用 `%Value` 参数/返回值通道**（零新机制），闭包隐藏 env 首参传入；零动态分发（函数值 = 直接函数指针，静态已知可内联）；捕获环境 = 捕获变量打包连续结构（复用 interp 捕获槽语义，三后端一致）。实施 = LLVM 后端 Phase 8 大改造，实现另计
