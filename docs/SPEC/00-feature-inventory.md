...
| 功能 | 描述 | 状态 |
|------|------|------|
| `type_is_send`/`type_is_sync` | 递归类型检查（标量自动 Send+Sync、`*mut` 非 Sync、集合看元素） | ✅ |
...
| 惰性迭代器 | 惰性/组合子迭代器 | ⏳ 1.x |
| 通用压缩算法 | gzip / zip 压缩 | ⏳ 1.x |
| 时区完整 | tz 库完整 | ⏳ 1.x |
| 真 OS 进程/共享内存 | 进程/共享内存完整 | ⏳ 1.x |
| 标准库数据结构缺口 | bitmap/侵入式链表/环形缓冲/树/页内存 | ⏳ 1.x |
| 数据库连接抽象 | 数据库连接抽象 | ⏳ 1.x |
...
| T5（E1–E2） | 元编程 + 并发完整 | 🟡 部分（元编程 E1 完整 + 并发 E2 完整含四模式/Send·Sync，OS 并行/mutex 1.x） |
| T6（E3–E5） | 标准库 + 工具链完整 | 🟡 部分（标准库 E3 扩展 + 系统编程 E4 + 工具链 E5 大部分落地，mismatch 归零 1.x） |
| T7（E7） | 自举闭环 | ⏳ |
| T8（E6 + 冻结） | 1.0 冻结 | ⏳ |
