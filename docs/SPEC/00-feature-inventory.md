...
| 语句解析 | 表达式 / 变量声明 / 常量声明 / if / while / for / switch（含守卫）/ return / break / continue / defer / errdefer / 块 | ✅ |
| 表达式解析 | 完整优先级：字面量 / 标识符 / 一元 / 二元 / 调用 / 索引 / 字段 / 取址 / 解引用 / if / switch / 闭包 / 数组/元组/命名构造 / 类型构造 | ✅ |
...
| 解释模式示例回归 | **147/148 通过 + 1 跳过**（0 失败） | 2026-08-23 |
...
| T5（E1–E2） | 元编程 + 并发完整 | 🟡 部分（Send·Sync 已落地，OS 并行/mutex 1.x） |
| T6（E3–E5） | 标准库 + 工具链完整 | 🟡 部分（B7 质量工具推进中，mismatch 归零 1.x） |
| T7（E7） | 自举闭环 | ⏳ |
| T8（E6 + 冻结） | 1.0 冻结 | ⏳ |
