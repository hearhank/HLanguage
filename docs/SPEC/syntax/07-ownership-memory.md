...
- 状态：⚠️ 规范已定（parser/诊断实现 → backlog #16）
- 证据：裁决 K1 + ADR-0036（2026-08-30）
...
- 规则：
  - **全局分配器 `alloc`**：`alloc.init(T)` / `alloc.init(T{...})` 构造；带**泄漏追踪**（按分配行号报告）；`alloc.init` 的配对销毁（`destroy`/`free`）**尚无**——已知缺口。
...
- 状态：⚠️ 部分——Arena bump/追踪 ✅；`alloc` 配对销毁 ⏳（已知缺口）；Arena 详细方法集核对归标准库文档
- 证据：`ir/models/cell.rs` L22-24（Arena G1 注释）；`ir/builtin.rs` `call_alloc_method_ir`（leak 追踪 L297-299）；`docs/SPEC/phase1/08-mem-allocator-design.md`
...
- 规则：
  ...
  - 显式销毁的调用形式随 alloc 配对 destroy（backlog #14①）确定。
...
- 已知缺口（实现待对齐 → backlog #14 组）：
  1. `alloc.init` 无配对 `destroy`
  2. 字段赋值不触发 move 检查（`doc.tag = move &tag;` 形态待实现）
  3. `join()`/`detach()` 不触发 move 检查（线程句柄）
  4. NonArena 自动跟踪已回退（按 AllocSource 判定替代）
