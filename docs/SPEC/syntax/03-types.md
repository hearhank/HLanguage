...
- 规则（裁决 F1，2026-08-30）：浮点 = **三个真实宽度** `f16` `f32` `f64`：
  - `f64`：✅ 已实现（当前唯一真实宽度）。
  - `f16`/`f32`：⏳ **升级为真实宽度的目标**（当前实现将 f16/f32/f128 全部别名映射 f64——升级涉及语义层 FloatWidth 划分、IR/VM 算术、LLVM 后端、字面量定型，见 backlog #5）；升级完成前写 `f32` 等价于 f64。
  - `f128`：❌ 废弃（F1 裁决未含 f128；如需 128 位浮点单独立项）。当前实现映射 f64，实现侧同步移除该名 → backlog #5。
  - `comptime_float` 字面量惰性定型 → 可定 f16/f32/f64。
  - 跨浮点宽度转换：显式 `@floatCast`（见 `13-builtins.md`）；无隐式转换。
- 证据：现状 `semantic/resolve.rs` L80-85（f16\|f32\|f64\|f128 → SType::Float，注释「H 浮点单一 f64 表示」）
...
- 规则：**无独立 char 类型**——字符字面量 = 单 ASCII 字节，类型 u8（`01` §1.5，裁决 D1）；非 ASCII 内容用字符串 `\u{...}` 或码点整数字面量。
- 状态：✅ 已实现（按 D1 裁决口径）
- 证据：`lexer/mod.rs` `lex_char`；`parser/ast.rs` `Expr::CharLit(u8)`
...
## 3.9 类型推断（Q-S9，经 H1 裁决修订 2026-08-30）

- 规则：
  - **必须显式**：函数参数类型（含 `*`/`*mut`/`owned` 形态）；**函数返回类型**（H1：必须标记，省略报错）；**变量绑定类型**（`var x: T = init`——推断 = ⏸ 自举后高级功能，在此之前注解必填）；class/struct 字段类型；接口实现标注；`owned` 不参与推断。
  - **字面量惰性宽度**：✅ 保留（字面量在使用处定型，如 `var a: i32 = 42` 的 42 定型 i32；超范围编译期报错）——属字面量定型，非变量推断。
  - **⏸ 自举后目标（变量类型推断，H1 裁决）**：变量绑定省略注解（`var x = init`）、指针形态推断（`var x = &mut t` → `*mut T`）等——属高级功能，自举（stage1/stage2）完成后实现；在此之前省略注解 = 编译错误（实现当前接受 → 规范收紧后补诊断，随 `00-index.md` 排除列表排期）。
  - 泛型参数推断（`anytype`/`where T`）属泛型实例化机制，不在本条（`06-interfaces.md`）。
- 状态：✅ 规范定案（实现侧注解必填诊断待补）
- 证据：`infer.rs`（Comptime 惰性定型）；裁决 H1（2026-08-30）；原 Q-S9 定案载 `06-01` 历史
