...
- **实现**：interp `make_four_mode_container`/`call_four_mode_method` + 三原子臂；IR 镜像 `make_four_mode_ir`/`call_four_mode_method_ir` + `is_type_arg_pos` 类型参数降级（`Pipe<i32>` 实例化）+ `call_builtin` 标记臂；semantic `is_builtin_type`（四模式名入内建类型）+ `call_at_builtin`（原子返回类型）。
