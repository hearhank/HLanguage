...
- ✅ **Send/Sync 编译期诊断（2026-08-22 定案，ADR-0017 C3-3；形态承 ADR-0016 #6）**：`Send`/`Sync` = **内建标记接口**（编译器内建实现，不可自定义）；**可推导性（组合性验证）**——标量/值类型自动 Send+Sync，指针/切片看指向类型，内建容器看元素/负载，用户 `class Foo: Send` 由编译器验证字段全满足才合法（含 `*mut` 或可变共享 → 非 `Sync`）；**诊断模式（协作式）**——`spawn`/`await` 边界捕获非 `Send` 引用 → 编译错误带位置（`captured value of type X is not Send at spawn boundary`），非 `Send` 值不可跨线程捕获；**与 Q19 正交**（Q19 冻结窗口管借用期，Send/Sync 管类型层跨线程可传递性，不替代 Q18/Q19）；**真并行检查（运行时）1.x 启用**——协作式下仅编译期诊断，零运行时开销
