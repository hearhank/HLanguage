...
- **参数**：数据栈对象（标量/连续类型）默认复制；其它对象只能 `*T`（只读引用）/ `*mut T`（可写引用）/  `owned T`（拥有，函数作用域接管，退出销毁）
...
- **move 规则**：仅本作用域拥有（非 Arena/global）的变量可 move；拥有参数用  `owned T` 标注（函数内隐含拥有，退出销毁）；`o` 与 `*`/`*mut` 正交（ `owned *mut T` 允许）；**调用点显式 `move`**（Q23）：`take(move s);` / `return move s;`
...
- **`box(value, alloc) -> owned *mut T`**：装箱——分配堆内存、值写入堆、返回带所有权的可写指针；标量可装箱为接口指针（`*INumber` 等，2026-08-14）
