...
**测试基线（2026-08-16）**：`hc` 前端 **35** + lib 单测 **25**（bytecode 往返/坏格式 7 + llvm.rs 纯文本发射 18，含 Phase 1/2 指针/聚合） + **bytecode 集成 14**（VM==参考一致性） + **IR 32**（M3.1/Phase 1 指针/Phase 2 聚合） + **inferred_errors 6** + `hc-rt` errors **18** + semantics **47** + **一致性 28**（M3.4 + Phase 1 指针 + Phase 2 聚合双模式） + io **6** + iter **4** + interfaces **10** + deque **4** + inference **11** + closures **4** + scalar **2** + serialize **4** + dep **3** + examples **41** + `hc-tools` 单测 **19** + native **19**（zig cc 端到端，含 Phase 2 聚合原生）全绿——`cargo test --workspace` **332** 全绿；`hc test examples/` 全目录 **124/135 通过**（11 失败分属第三块 E1/E2 特性，见下）。
...
**已知取舍**：字节码 VM 执行复用 `run_ir`（`IrInst` 枚举 + `IrValue` 盒式表示，未做紧凑运行时 dispatch / 寄存器式 VM——性能优化留后续，须一致性套件证明等价）；`hc build` 原生编译为 emit-.ll + `zig cc`（LLVM 依赖外部 zig；i64 载荷/字符串字面量子集/无优化/硬错误消息依赖 libc）；u64 移位按 64 位截断（xorshift 语义）；闭包捕获整个作用域链（自由变量精确分析留后续）；原生/IR 后端为**标量 + 指针 + 聚合子集**（字段/索引/切片/数组/class/enum/元组解构/move/unwrap 已覆盖）——子集外特性（for/switch/defer/闭包/集合/class 方法/全局变量等）在 IR 降级时以 `error.Unsupported` 硬错误拒绝（不静默丢弃，`hc build`/`hc run --ir` 报错并提示改用 tree-walking 模式）；指针 `&非lvalue` 快照在原生后端指向临时槽（帧存活期），逃逸出帧为悬垂（IR/VM 中为独立 cell 无此问题）——`&变量` 别名路径无此差异；tree-walking 求值递归栈深——`hc run` 与示例回归测试均在 64MB 栈线程中运行（主线程默认栈不足，非语义限制）；`hc test --mode=compile` 原生交叉验证为**文件粒度**（全绿 vs 有失败），非逐测试 PASS/FAIL 清单——断言失败在测试函数 ret 路径直接 abort，逐测试续跑需重做 assert→返回码通路。
