...
**测试基线（2026-08-16）**：`hc` 前端 **34** 单测（13 原有 + 9 M2.6 + 7 M2.4 + 4 M1.4 + 1 ArrayLit 尾逗号） + **IR 23**（M3.1） + **bytecode 7**（单测，往返/坏格式）+ **bytecode 集成 12**（VM==参考一致性） + `hc-rt` errors **18** + semantics **47**（13 原有 + 24 M2.2 + 3 M2.5 + 7 M4.3） + **io 6**（net echo/帧/fs seek/时间/环境/连接拒绝） + **一致性 14**（M3.4） + **dep 3**（M7.2 跨包/pub 边界） + **hc-tools 19**（含 `merge_modules`/`strip_test_funcs`/`test_runner` 单测 + native 端到端，M3.3/Q-T5）全绿；`hc test examples/` 全目录 **122/134 通过**（12 失败分属第三块 E1/E2 特性 + 接口错误契约，见下）。
...
**示例验收说明（2026-08-16）**：剩余 12 个失败示例分属——E1 元编程（35-comptime-branch、34-generics、63-template-render）、E2 并发/异步（37-concurrency、38-async、39-evented、76-threads-edge、77-producer-consumer、78-task-dispatch、79-retry、80-batch-async）、接口错误契约（24-interface-errors 引用未实现 json/csv 库）——均为第三块（第二部分）特性或未实现库，属已知失败；25-error-context/26-error-set-union 跨文件泛型重载碰撞已修复（2026-08-16 梯队 16 语义 `skip_entry` 去重，转 PASS）。

**已知取舍**：字节码 VM 执行复用 `run_ir`（`IrInst` 枚举 + `IrValue` 盒式表示，未做紧凑运行时 dispatch / 寄存器式 VM——性能优化留后续，须一致性套件证明等价）；`hc build` 原生编译为 emit-.ll + `zig cc`（LLVM 依赖外部 zig；i64 载荷/字符串字面量子集/无优化/硬错误消息依赖 libc）；u64 移位按 64 位截断（xorshift 语义）；闭包捕获整个作用域链（自由变量精确分析留后续）；原生/IR 后端为**标量子集**——子集外特性（for/switch/defer/闭包/集合/指针/字段/索引/全局变量等）在 IR 降级时以 `error.Unsupported` 硬错误拒绝（不静默丢弃，`hc build`/`hc run --ir` 报错并提示改用 tree-walking 模式）；tree-walking 求值递归栈深——`hc run` 与示例回归测试均在 64MB 栈线程中运行（主线程默认栈不足，非语义限制）；`hc test --mode=compile` 原生交叉验证为**文件粒度**（全绿 vs 有失败），非逐测试 PASS/FAIL 清单——断言失败在测试函数 ret 路径直接 abort，逐测试续跑需重做 assert→返回码通路。
