...
> **2026-08-17 梯队 28 更新（P11a/b `math.*` 内建族 IR + LLVM 原生落地：compile mismatch 68→58）**：按「LLVM 原生标准库补齐」阶段路线首块落地——① **`hc/src/ir.rs` 降级**：`is_type_arg_pos`（2547）为 `math.nan`/`math.inf`/`math.inf_neg` 标 i==0 为**类型名参数**（运行时忽略、仅指示宽度——与 oracle `call_math` 一致：nan/inf/inf_neg 直接返回 Float 常量、不读取类型名参数）；`is_dotted_implicit_root`（4225）加入 `"math"`（与 json/csv/String 同机制虚拟根——**不加 IMPLICIT_ENV**，否则 `resolve("math")` 返回 Some 误路由 `CallMethod`，须保持 `Call` 路径）；`call_dotted_implicit`（6116）math 臂：nan/inf/inf_neg → Float 常量，sqrt/abs/pow/floor/ceil/round → `deref_value` args[0] + Int→f64 强转 + 计算（pow = `f.powf(2.0)` 单参平方，对齐 oracle）；② **`hc/src/llvm.rs` 原生 codegen**：declare `floor`/`ceil`/`round`（fabs/sqrt 已有）；`math_unop_helper(fname, op)` 生成 Int→f64（trunc i128→i64→sitofp / bitcast / select）+ op + 回 Float（`%Value = {i32 tag, i128 data}`，T_FLOAT=3，data = `f.to_bits()` 低 64 位）；注册 `hc_abs`（fabs）/`hc_floor`/`hc_ceil`/`hc_round`/`hc_pow`（`%r = fmul double %f, %f`）；`call()` 加 `name.strip_prefix("math.")` 分支 → `call_math`（nan/inf/inf_neg 直接 `build_store` f64 bit 常量：NaN=9221120237041090560、inf、inf_neg）；③ **示例 07-floats 消除**：`math.nan(f64)`/`math.inf(f32)`/`math.inf_neg(f64)` 不再「未知标识符」——02-types 目录 16→6 mismatch（9 个被组毒化的兄弟文件 + 07-floats 全部 MATCH）。**测试**：`hc-rt/tests/consistency.rs` **61→62**（`p7_math_builtins`：nan!=nan、sqrt(4.0)==2.0、pow(3.0)==9.0、floor/ceil/round、sqrt(9)==3.0 Int 强转，双模式 PASS/FAIL 一致）、`hc/src/llvm.rs` 纯文本发射 **35→36**（`phase7_math_builtins_emit_nan_and_helpers`：i128 9221120237041090560 + hc_sqrt/hc_pow 调用 + helper 定义）。**基线**：`cargo test --workspace` 全绿（**354** 单元/集成 + 示例回归，consistency 62 全过）；示例回归 interpret **125/136 不变**；compile **68 → 58 mismatch**——10 例转 MATCH，剩余 58 分类：12 defer 体控制流（`defer try f()` 设计内硬错误）+ 9 struct 字面量（6 Orders.Line + 3 struct）+ 37 运行时（19 NotBuiltin + 5 NoMethod + 4 AssertFailed〔2 连续类拷贝别名 + 1 原生重载类型分派 + 1 30-interface〕+ 3 NotCallable + 3 NoField）。**gate 更新**：`tag1/scripts/check-examples.sh` compile 阈值 **≤68 → ≤58**、`tag1/README.md` 基线同步。**P11a/b 验证对照**：计划「LLVM 原生标准库补齐，按阶段推进、逐阶段可交付」以 math.* 端到端（07-floats MATCH + 新一致性/发射测试 + gate 收敛）落地；下一步 P11d 从连续类 DeepCopy 指令（13-struct/58-copy-semantics 别名）与原生重载类型分派（87-overloads）开始。

> **2026-08-17 梯队 29 更新（P11d 首子项：连续类值语义 DeepCopy 指令贯通 IR/字节码 VM/LLVM 原生；compile mismatch 58→57）**：按「剩余 mismatch 类别归零」阶段首子项落地——**连续类 var 声明即复制（对齐 oracle `interp.rs:926-949`）**：`var p2: Point = p` 此前在 IR/原生经 `Store` 克隆 cell 句柄 → `p2.x = 99` 写穿 p → `error.AssertFailed`。修复以**运行时门 DeepCopy 指令**落地——① **`hc/src/ir.rs`**：`IrModule.continuous: HashSet<String>`（`collect_types` 捕获 `Decl::Class` 的 `traits` → `ClassInfo.continuous`，`lower()` 汇总扁平类名）+ `IrInst::DeepCopy{temp,a}` + `needs_deep_copy` 门（标注类型 `Type::strip`→Named 且类连续，或未标注 + `Expr::Ident` 初始化保守发射）+ `exec_body` 臂（`is_continuous_class` 运行时按值类名 ∈ `module.continuous` → 递归 `deep_copy`，否则恒等=引用别名）；**Oracle 语义验证**：`Type::strip` 仅解包 Owned——`?Point`/`!void` 非 Named 恒不复制，非连续类/数组恒等别名（语义层禁按值赋值非连续类——`copy(&x)` 显式深拷贝）；② **`hc/src/bytecode.rs`**：HBC2 **v6 + opcode 47** + 模块尾部 `[continuous]` 类名表 encode/decode 对称（`merge_modules` 跨文件并集）；③ **`hc/src/llvm.rs`**：递归 `hc_deep_copy` helper（Arr/Class/Str/Ptr 分支，对齐 oracle deep_copy；Closure 原生不存在、Str 为不可变全局恒等）+ 每模块门 `hc_deep_copy_cont`（tag==10 类检查 + **strcmp 链**比对连续类名——LLVM 18 禁止常量表达式 GEP，全局指针数组不可行）+ `DeepCopy` 臂（load→call→store）。**测试**：`hc-rt/tests/consistency.rs` **62→63**（`continuous_class_value_semantics_consistency`：标注类型连续复制 + 未标注标识符初始化复制，双模式 PASS 一致）、`hc/tests/ir.rs` **68→69**（`continuous_class_copy_on_var_decl` 直接 run_ir 断言）、`hc/src/bytecode.rs` 往返夹具 + `continuous` 表断言（opcode 47）、`hc/src/llvm.rs` 纯文本发射 **36→38**（`continuous_class_copy_emits_deep_copy_gate`：`hc_deep_copy_cont`/`hc_deep_copy` 定义 + 调用 + "Point" 字符串表 + strcmp 链；`non_continuous_class_no_deep_copy_inst`：无连续类模块恒等门无递归 helper）。**基线**：`cargo test --workspace` 全绿 **456**（HEAD e2f6e1d 实测；提交信息误记 452）；示例回归 interpret **125/136 不变**；compile **58 → 57 mismatch**——13-struct（`pure_value_copy`）转 MATCH，58-copy-semantics 的 `scalar_and_continuous_copy` 原生已 PASS（文件仍 MISMATCH 系 `collection_explicit_copy` `error.NotBuiltin`：`copy(&Vec)` 聚合深拷贝原生未实现，归运行时内建类别）。**gate 更新**：`tag1/scripts/check-examples.sh` compile 阈值 **≤58 → ≤57**、`tag1/README.md` 基线同步。**P11d 验证对照**：计划「2 AssertFailed（连续类拷贝别名）归零」以连续类深拷贝三处（13-struct + 58-copy 两测试）原生 PASS 落地；剩余 57 构成：12 defer-try-f（设计内硬错误，`defer try f()` 体不允许控制流）+ 9 struct 字面量（6 `Orders.Line` 跨包命名类 + 3 匿名 `struct`）+ 36 运行时（22 NotBuiltin〔含 58-copy `copy(&Vec)`/`copy(&String)` 聚合深拷贝原生未实现〕+ 5 NoMethod + 3 NotCallable + 3 NoField + 3 AssertFailed〔87-overloads 原生重载类型分派、30-interface、31-class〕）。

> **2026-08-17 梯队 30 更新（P11d 次子项：隐式环境全局原生播种；compile mismatch 57→54）**：继续「剩余 mismatch 类别归零」——**30-interface `Circle.area` 原生读 `pi` 得 0.0 的根因**：`@.h_globals` 为 zeroinitializer 而 `@__init__` 只播用户全局；`IMPLICIT_ENV`（pi/alloc/io/test_io/stdout/stderr/Vec/Deque/Map/Table，ir.rs:4292）在 run_ir 由 `IrRuntime::init` 经 `implicit_env_value` 播种（pi→Float(PI)、Vec/Deque/Table→空 Arr、io 族→Io 实例），原生侧此前全为 Void。修复以 **`emit_implicit_env_seed`**（llvm.rs:3389）在 `emit_main_wrapper`/`emit_test_runner` 入口（`@__init__` 前）对每全局槽发常量/helper store：pi→Float(PI) 位模式常量（`store %Value { i32 3, i128 4614256656552045848 }`）、Vec/Deque/Table→`hc_make_arr(i64 0)`、io/test_io/stdout/stderr→`hc_make_io()`（helper 恒发射——emit_aggregate_helpers/emit_io_helper 无条件）。**测试**：`hc/src/llvm.rs` 纯文本发射 **38→39**（`implicit_env_pi_seeded_in_entry`：pi Float store + `[10 x %Value]` GEP 槽 5 + `hc_make_arr(i64 0)`）。**基线**：`cargo test --workspace` 全绿 **457**（HEAD 实测 456 → +1 发射测试）；示例回归 interpret **125/136 不变**；compile **57 → 54 mismatch**（30-interface 转 `[MATCH]`；另本会话调试临时文件 2 例移除——此前混入计数）。**当前 54 构成**：21 Unsupported（12 defer-try-f 设计内硬错误〔64-75，`defer try f()` 体不允许控制流〕+ 6 `Orders.Line` 跨包命名类字面量〔40-io/41-namespaces/42-pricing/43-orders/44-multi-file-main/math〕+ 3 匿名 `struct` 字面量〔33-script/34-generics/36-script-boilerplate〕）+ 33 运行时（22 NotBuiltin + 6 NoMethod + 3 NotCallable + 2 AssertFailed〔87-overloads 原生重载类型分派、31-class〕）。**gate 更新**：`tag1/scripts/check-examples.sh` compile 阈值 **≤57 → ≤54**、`tag1/README.md` 基线同步。**P11d 验证对照**：计划「3 AssertFailed（87-overloads/30-interface/31-class）归零」以 pi 种子修复 30-interface 转 MATCH 落地首项；下一步原生重载类型分派（87-overloads）与 31-class。

> **2026-08-17 梯队 31 更新（P11d 第三子项：原生集合方法——Arr 接收者 `Vec.append` 族落地；compile mismatch 54→52）**：继续「剩余 mismatch 类别归零」——**31-class/46-recursion 原生 `Vec.append` NoMethod 消除的根因**：原生 `call_method`（llvm.rs）在 `owners.is_empty()` 时直接 `abort_feature("nomethod")`，只分派 Class（tag 10）接收者；Arr（tag 8）接收者的内建集合方法（`append`/`push_back`/`append_u64`/`extend`/`init`/`len`）无分派路径 → `tree_recursive_composition`/`tree_depth`（`root.children.append(...)`）原生 NoMethod 响亮中止。修复以三部分落地：① **`call_method` 重构**：加 `let is_coll_method = matches!(method, "append" | "push_back" | "append_u64" | "extend" | "init" | "len")`；早退改 `if owners.is_empty() && !is_coll_method`；统一 `l_done` 合并块——集合方法 `br i1 is_arr, label %l_coll, label %l_cls`（coll 块 `call_coll_method` 后 `br %l_done`，l_cls 落入既有 Class 分派链），owner 链命中块 `br %l_done`；② **新增 `call_coll_method`**：per-method 发射——`init`→`hc_make_arr(i64 0)`；`len`→ArrObj.len 提为 Int；`append`/`push_back`→`call void @hc_append(dv, v)`；`append_u64`→`hc_append_u64`；`extend`→`hc_extend`；void 方法 store `{i32 0, i128 0}` 到 temp；③ **三个运行时 helper 常量 + 发射**（`HC_APPEND`/`HC_APPEND_U64`/`HC_EXTEND`，注册进 `emit_aggregate_helpers` 于 `HC_ARR_SET` 后）——**`hc_append` 原地扩容**：`hc_alloc` 新 items 缓冲 + phi-loop 拷贝旧元素 + 追加 + `store %ArrObj` 回**同一堆指针**（`root.children.append(x)` 字段别名共享同一 ArrObj 堆指针，append 后所有别名见增长——对齐 run_ir 槽 cell 语义）；`hc_append_u64` 8 字节小端展开逐 append（xorshift 写字节场景）；`hc_extend` 分派 Str（strlen+字节循环）vs Arr（tag 8 元素循环）。**测试**：`hc/src/llvm.rs` 纯文本发射 **39→40**（`aggregate_append_emits_coll_helpers`：`Node.new(1, alloc)` + `r.children.append(...)` + `return r.children.len`，断言 `define void @hc_append`/`@hc_append_u64`/`@hc_extend` + `call void @hc_append`）。**基线**：`cargo test --workspace` 全绿 **458**（457→458：+1 发射测试，逐文件实测 hc lib 40 / ir 69 / consistency 63）；示例回归 interpret **125/136 不变**；compile **54 → 52 mismatch**——31-class/46-recursion 转 `[MATCH]`。**当前 52 构成（逐文件原生中止码实测）**：21 Unsupported（12 defer-try-f 设计内硬错误〔64-75〕+ 6 `Orders.Line` 跨包命名类字面量〔40-io/41-namespaces/42-pricing/43-orders/44-multi-file-main/math〕+ 3 匿名 `struct` 字面量〔33-script/34-generics/36-script-boilerplate〕）+ 31 运行时（20 NotBuiltin + 6 NoMethod〔86-scalar-interfaces 标量方法族 / 20-errors·25-error-context·26-error-set-union fs·文件方法 / 50-serialization·57-protocol-parse 序列化 to_bytes·from_bytes〕+ 3 NotCallable〔10-functions/21-closures/48-iterator-chain〕+ 2 AssertFailed〔87-overloads 原生重载类型分派、84-rng rng_range〕）。**gate 更新**：`tag1/scripts/check-examples.sh` compile 阈值 **≤54 → ≤52**、`tag1/README.md` 基线同步。**P11d 验证对照**：计划「NoMethod 类别归零」以集合方法族（31-class/46-recursion）落地首项；剩余 NoMethod 均为独立大面（标量接口族 / fs·文件方法 / 序列化 to_bytes·from_bytes 原生实现），是否继续归零须授权。**【2026-08-17 用户裁定：P11d 到此收束】**——compile mismatch 收敛至 **52** 为本阶段（LLVM 原生内建子集扩展）边界：剩余 52 构成（逐文件原生中止码实测，已记录于上）中，21 Unsupported 为设计内硬错误/降级（defer 体控制流 12 + `Orders.Line` 跨包字面量 6 + 匿名 struct 3），31 运行时为独立原生内建大面（NotBuiltin 20 / NoMethod 6 / NotCallable 3 / AssertFailed 2），均留后续阶段（原生 ABI 全标准库），不再逐类归零。

> **2026-08-17 梯队 32 更新（第二部分组 B：io.print 格式说明符补齐 + 静默输出修复，双模式对齐）**：09 执行细表 B 组三项落地——① **B1 `interp.rs`**：`call_io_print` 重写为通用说明符解析（新 `format_spec_value`：`{}` 默认 / `{d}` / `{x}` / `{X}` / `{b}` / `{e}` / `{s}` + Zig 式宽度/对齐 `<`/`>`/`^`/精度 `{:8}`/`{:<6}`/`{:.2}`——05-format.hc 清单全支持，interp 输出正确）；② **B2 静默输出修复**：未知/非法说明符（多字符残留或未识别类型字符）→ `error.FormatError` 运行期错误（不再按字面量静默输出），interp 与 IR 双模式一致失败；③ **B3 IR/原生对齐**：`hc/src/ir.rs` `format_spec_value_ir` 镜像 interp（`call_io_print_ir` 走同一解析）；`hc/src/llvm.rs` `parse_print_fmt` 重写——mode 扩为 4（`{X}` 大写十六进制）/5（`{e}` 科学计数），`hc_write_u128_base` 加 `%upper` 参数（≥10 位偏移 87→55 select）、`hc_write_int` mode 4 → base 16 + upper、`hc_write_value` float_case 加 mode 5 `%e` 分支（printf `%e`，新增 `@.fmt_e` 常量）+ 注释记录取舍（原生宽度/精度/对齐暂不填充、`%e` 固定 6 位小数与 Rust `{f:e}` 最短表示有差异——均无示例/测试比对精确输出）。**测试**：`hc-rt/tests/consistency.rs` **63→65**（`b1_print_format_specifiers` 双模式执行无错 + `b2_unknown_format_specifier_fails_both` 双模式一致失败）。**基线**：`cargo test --workspace` 全绿；示例回归 interpret **125/11**、compile **52 mismatch** 不变（05-format 原生 `{X}`=FF 与 interp 一致；宽度/精度差异为既有取舍，仍计 52 内）。**B 组验证对照**：计划「B1 05-format 输出正确 / B2 负例绿 / B3 consistency + native 绿」以 interp 05-format 全清单正确 + b2 负例双模式失败 + 原生 `{X}`=FF 端到端落地。
