...
**测试基线（2026-08-17）**：`hc` 前端 **35** + lib 单测 **31**（bytecode 往返/坏格式 7 + llvm.rs 纯文本发射 24，含 Phase 1/2 指针/聚合 + Phase 3 switch/for） + **bytecode 集成 20**（VM==参考一致性，含 Phase 3 六新 opcode 往返） + **IR 38**（M3.1/Phase 1 指针/Phase 2 聚合/Phase 3 switch+for） + **inferred_errors 6** + `hc-rt` errors **18** + semantics **47** + **一致性 41**（M3.4 + Phase 1 指针 + Phase 2 聚合 + Phase 3 switch/range/for 双模式） + io **6** + iter **4** + interfaces **10** + deque **4** + inference **11** + closures **4** + scalar **2** + serialize **4** + dep **3** + examples **41** + `hc-tools` 单测 **19** + native **27**（zig cc 端到端，含 Phase 2 聚合 + Phase 3 switch/for 原生）全绿——`cargo test --workspace` **371** 全绿；`hc test examples/` 全目录 **125/136 通过**（11 失败分属第三块 E1/E2 特性，见下）。
