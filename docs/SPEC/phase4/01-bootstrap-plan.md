...
> **2026-08-26 性能优化**：`peek()`/`peek_text()` 返回 `&[u8]` 避免 Vec 拷贝，全局 `kind_eq` 替换为 `==` 切片比较，解析自身从 60s+ 降至约 1s（~60x 提升）。8 项语料对照通过，1 项已知不支持（12-if-while.hc += 复合赋值）。
...
### 🟢 K2 性能已优化：解析自身约 1s（原 60s+，提升 ~60x）

K2 性能优化已完成（2026-08-26）：
- 将 `peek()`/`peek_text()` 返回类型从 `Vec<u8>` 改为 `&[u8]`，避免 Vec 拷贝
- 将 `at()` 改用 `==` 切片比较，消除 `kind_eq` 函数调用开销
- 将 `expect_name_or_keyword()` 返回 `&[u8]` 替代 `Vec<u8>`，减少分配
- 移除 `kind_eq()` 函数，全局替换为 `==` 切片比较
- 对照测试 2 passed / 1 ignored（自举验证需手动运行）
...
| 编号 | 功能 | 状态 | 出处 |
|---|---|---|---|
| Z1 | K2 H 版 parser / AST（token → 声明树）+ 对照测试 | 🟢 性能已优化（1s vs 原 60s+） | 原 `10-part3-execution.md` 组 K |
| Z2 | K3 H 版语义（名称解析 / 类型 / 所有权 / 错误集）+ 对照 | 🔴 | 组 K |
| Z3 | K4 H 版后端（AST 树遍历执行引擎，跑 H 程序）+ 对照 | ✅ 2026-08-29（10/10 语料与 Rust 参考逐字节一致） | 组 K |
