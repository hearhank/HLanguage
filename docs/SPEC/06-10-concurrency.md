...
>
> **状态（2026-08-18 组 E 落地后更新）**：`async`/`await`/`Future(T)`/`Io.threaded()/evented()` 已随组 E（E2.3 异步，协作式 Future）落地（下面标注 ✅）；**四模式类型（OneToOne/OneToMany/ManyToOne/ManyToMany）、真 OS 并行、`@atomic*`、`mutex` 按 ADR-0011 延迟 1.x**（需真 OS 线程语义，本块不实现）。
