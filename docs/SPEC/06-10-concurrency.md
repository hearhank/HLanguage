...
- **线程所有权**：spawn 归当前作用域；退出时已完成→销毁、运行中→移交根作用域（**无隐式阻塞**）；**根作用域 = 程序最后退出场所**，负责最终资源回收（评审 A7）；显式 `join() !T` 消耗所有权，错误以 error union 跨线程传播
...
- **Future/Thread 方法集一致（2026-08-14 定案）**：`join() !T` / `cancel() !void`（**协作式取消**——任务在 await/IO 点检查取消标志，非强杀）/ `is_done() bool` / `detach()`；`await f` ≡ `f.join()`
