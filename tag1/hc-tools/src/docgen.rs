...
const STDLIB: &[(&str, &[(&str, &str)])] = &[
    (
        ...
        &[
            ...
            ("io.exit(t: ExitType, code: u8) !void", "退出：Exit 静默 / Error 打印标记"),
            ("io.net.get(url: String) !String", "HTTP GET"),
            ("io.fs.read(path: String) !String", "读文件"),
            ("io.fs.write(path: String, data: String) !void", "写文件"),
        ],
    ),
    (
        "alloc / mem（内存分配）",
