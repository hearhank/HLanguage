...
pub(crate) mod buildzon;
pub(crate) mod fsio;

/// H1：`hc init <name>`——在当前目录生成最小项目骨架（`build.zon` + `src/` + `docs/`）。
///
/// 2026-08-23 重构：使用 `src/` 子目录存放源码，添加 `docs/`、`README.md`、`version.hc`。
/// 约定见 `docs/SPEC/06-13-project-structure.md`：目录 = 包、源码位于 src/、
...
pub(crate) fn init_project(name: &str) -> ExitCode {
    ...
    if let Err(e) = std::fs::create_dir_all(&src_dir) {
        eprintln!("error: 创建 src/ 目录失败: {e}");
        return ExitCode::FAILURE;
    }
    if let Err(e) = std::fs::create_dir_all(&docs_dir) {
        eprintln!("error: 创建 docs/ 目录失败: {e}");
        return ExitCode::FAILURE;
    }
    let zon = format!(
        "// build.zon — {name} 包清单（hc init 脚手架）\n\
         ...
         //   - author/description：项目元数据（可选）\n\
         //   - files：包内文件清单（源码位于 src/ 目录）\n\
         ...
         \x20   version = \"0.1.0\",\n\
         \x20   kind = Kind.exe,\n\
         \x20   // author = \"your name\",\n\
         \x20   // description = \"A short description\",\n\
         \x20   // license = \"MIT\",\n\
         \x20   files = [ \"src/main.hc\", ],\n\
         \x20   deps = [],\n\
         }};\n"
    );
    let main_hc = format!(
        "import H.std.{{io}};\n\
         \n\
         // {name}/src/main.hc — 项目入口（hc init 脚手架）\n\
         //\n\
         //   - 源码约定：`.hc` 文件位于 src/ 目录（目录 = 包，M1.4）\n\
    ...
    let version_hc = format!(
        "// {name} version info\n\
         // Auto-generated by hc build\n\
    ...
    if let Err(e) = std::fs::write(src_dir.join("main.hc"), &main_hc) {
        eprintln!(
            "error: 写入 {} 失败: {e}",
            src_dir.join("main.hc").display()
        );
        return ExitCode::FAILURE;
    }
    if let Err(e) = std::fs::write(dir.join("README.md"), &readme) {
        eprintln!("error: 写入 README.md 失败: {e}");
        return ExitCode::FAILURE;
    }
    if let Err(e) = std::fs::write(dir.join("version.hc"), &version_hc) {
        eprintln!("error: 写入 version.hc 失败: {e}");
        return ExitCode::FAILURE;
    }
    println!("创建项目 `{name}`：");
    println!("  {name}/build.zon");
    println!("  {name}/src/main.hc");
    println!("  {name}/docs/");
    println!("  {name}/README.md");
    println!("  {name}/version.hc");
...
pub(crate) fn pkg_publish() -> ExitCode {
    ...
    let manifest = match buildzon::parse(&src) {
        Ok(m) => m,
        Err(e) => {
    ...
    let manifest_zon = format!(
        ...
        manifest.version,
        match manifest.kind {
            buildzon::Kind::Exe => "Kind.exe",
            buildzon::Kind::Lib => "Kind.lib",
            buildzon::Kind::Script => "Kind.script",
        },
        files
            .iter()
            .map(|f| {
