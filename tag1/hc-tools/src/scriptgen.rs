//! 装载期处理：comptime 块求值 + `.hs` 脚本缓存工具 + 命名空间自动推断。
//!
//! 脚本展开（`script { }` 块）已从 `.hc` 中移除（2026-08-23 定案，
//! `docs/SPEC/phase3/12-script-redesign.md`），`.hs` 文件成为唯一的脚本机制。
//! 本模块保留 comptime 块求值、`.hs` 脚本缓存和命名空间自动推断（M1-1）。
...
fn get_project_name_from_root(project_root: &Path) -> String {
    crate::buildzon::load_from_dir(project_root)
        .ok()
        .flatten()
        .map(|m| m.name)
        .filter(|n| !n.is_empty())
