//! hc fmt —— token 级格式化（I1）
//!
//! 以 hc lexer 的 token 流 + span 切片重建源码：缩进/换行/空格规范，AST 保真
//! （token 序列不变 → 输出必能重新解析为等价程序）。注释被 lexer 丢弃，
//! 从 token 间隙提取并按「独立行 / 行内块注释 / 行尾注释」三类保留。
//!
//! 排版规则（对齐 H 示例风格）：
//! - 4 空格缩进；`{` 位于行尾，块内容缩进一级，`}` 独占行（`} else` / `} ,` 除外）
//! - `;` 结尾换行；二元运算符两侧空格；`,` 后空格；`:` 类型标注后空格
//! - `0..10` 范围、`p.x` 字段、`f(x)` 调用、`[5]i32` 数组类型、`!void` 错误联合均不空格
//! - struct 字面量 `Point{x = 1.0, y = 2.0}`（Q6，大括号紧贴类型名）；`import H.std.{io}`（均不空格）
//! - `for (arr) |item|` / `while (c) : (s)` / `switch (v) { pat => body, }` 臂尾逗号换行
...
impl<'a> Fmt<'a> {
    ...
    fn push_comment(&mut self, cs: usize, ce: usize) {
        ...
        let own_line = {
            ...
            self.src[line_start..cs].chars().all(|c| c == ' ' || c == '\t')
        };
        let followed_by_code = {
            let line_end = self.src[ce..].find('\n').map(|p| ce + p).unwrap_or(self.src.len());
        ...
        self.comments.push(Comment { start: cs, end: ce, text, kind, line });
    }
    ...
    fn emit_gap(&mut self, offset: usize) {
        ...
        loop {
            let next_ci_start = self.comments.get(self.ci).map(|c| c.start).unwrap_or(offset);
        ...
        if nl_count >= 2 {
            ...
            && ((!self.brackets.is_empty()
                && *self.bracket_multiline.last().unwrap_or(&false))
                || (!self.parens.is_empty()
                    && *self.paren_multiline.last().unwrap_or(&false))
                || (!self.braces.is_empty()
                    && *self.brace_multiline.last().unwrap_or(&false)))
        {
    ...
    fn emit_token(&mut self, kind: &TokenKind, text: &str, next: Option<&TokenKind>) {
        match kind {
                ...
                match &popped {
                    Some(ParenKind::Control(kw)) => match *kw {
                        ...
                        "if" => {
                            if matches!(
                                next,
                                Some(TokenKind::LBrace) | Some(TokenKind::Pipe) | Some(TokenKind::Colon)
                            ) {
                                self.force_block = true;
                            }
                        }
            ...
            TokenKind::LBrace => {
                ...
                let empty = matches!(next, Some(TokenKind::RBrace))
                    && !self.cur_brace_has_comment();
            ...
            TokenKind::Pipe => {
                if self.in_capture {
                    ...
                    if open_capture {
                        ...
                        if !matches!(self.prev, Some(TokenKind::LParen) | Some(TokenKind::LBracket))
                        {
                            self.space();
                        }
                        self.raw("|");
    ...
    fn bracket_is_type(&self) -> bool {
        ...
        let after_is_type =
            matches!(after, Some(TokenKind::Ident(_)) | Some(TokenKind::LBracket));
