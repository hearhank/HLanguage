...
use crate::pkg::programs_to_test_ll;
use crate::project::fsio::{collect_hc_files, link_exe, zig_cc_available};
...
pub(crate) fn test_dir_dangle(target: &Path, mode: TestMode, dangle: DangleMode) -> ExitCode {
    ...
    if target.is_dir() {
        crate::cli::versiongen::bump_version(target);
    }
