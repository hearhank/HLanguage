...
`cargo test --workspace` 共 **791 项测试**（748 单元/集成 + 43 示例回归），全部通过。逐测试文件明细：
...
| hc-rt | `tests/consistency.rs`（M3.4 双模式一致，含 Phase 1–8 + 组 G 线程 + 组 D 值函数 + 组 E async + 组 G 标准库 G1-G5 + 组 H union/volatile/ptr/export） | 103 |
...
脚本生成（`script` 块元编程）、comptime 完整（类型即值）、并发/异步/线程、标准库扩展（UDP/HTTP/ipc/FFI 等）、系统编程（K1–K11 —— 组 H 已落地 K1 无标签 union / K2 volatile / K4 整数↔指针 / K5 export fn，剩 K3 asm / K6 freestanding / K7–K11 1.x 候选）、工具链扩展（LSP/format/lint/注册中心）、**自举**（stage1 → stage2）—— 详见 `07-bootstrap-plan.md` 第四节。
