// ============================================================
// stage1/parser.hc — H 版 parser（K2，E7 自举渐进路线 · 语法阶段）
//
// 双实现对照：本文件与 Rust 参考 parser（tag1/hc/src/parser/，`hc parse` 转储）
// 输出同一树格式（深度缩进 + NodeType|key=val），逐行 diff，差异即 bug。
// Rust 参考实现长期保留（自举失败风险对策）。
//
// 用法：hc run stage1/parser.hc <file.hc>
//
// 输出格式（与 Rust `hc parse` 一致）：
//   Program
//     Fn|name=main
//       Param|name=x|ty=i32
//       ret: !void
