...
按**使用层次 + 语法子类**分类，全局连续编号 01–85（沿目录顺序递增）+ 86–88/90/91（后续阶段补编，如标量接口/重载/迭代器/线程生命周期/模块 domain）。每类一个文件夹。
...
81-end-to-end · 82-directory · 83-wordcount · 84-rng · 85-grep-tool · 91-orders-domain

> 示例性质：语法规格示例（设计文档形态），非可运行程序——编译器实现时逐文件作为语法验收测试。91-orders-domain 为 `[module]` 领域模块约定（组 H3）示范：边界（owns 数据 + pub API）+ 上下文（init 参数列表依赖注入）+ 隔离（限定访问）。
