...
//
//   - 任务队列：Funnel<i32>（单写者分发 + 多读者 worker）
//   - 结果汇合：Tee<i32>（多写者 worker + 单读者主线程）
//   - 容器方法取 *Self（内建共享特例，Q32）；作用域绑定可捕获引用（Q18）

fn worker(tasks: *Funnel<i32>, out: *Tee<i32>) void {
...
fn main() !void {
    var tasks = Funnel<i32>.init(alloc);
    var out = Tee<i32>.init(alloc);
