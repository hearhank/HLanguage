...
class Config {   // 含 &[u8] 字段 → 非 Continuous（默认 class，堆上）
    host: &[u8],
    port: i32,
    timeout_ms: i32,
}
...
fn default_config() Config {
    return alloc.init(Config{host = "localhost", port = 8080, timeout_ms = 1000});
}
...
fn load_config<T>(io: *T, path: &[u8]) ConfigError!Config where T: Io {
    ...
    return alloc.init(Config{                              // 字段缺失 → 默认值（orelse 链）
        host = json.get("host") orelse "localhost",
...
[Test] fn config_load_defaults() !void {
    ...
    try expect_eq_slices(cfg.host, "localhost");
}