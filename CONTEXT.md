...
4. **没有隐藏控制**：无异常、无宏、无隐式内存分配、无隐式控制流；资源释放显式（`defer`/`errdefer`，ADR-0025 废除作用域自动销毁）；所有权转移显式（`owned`/`move`，ADR-0036）；错误路径显式（`!T` + `try`/`catch`）。隐式白名单仅两项：**局部变量类型推断**（仅 `var` 且可从初始化器无歧义推断，D8/ADR-0037）与**字面量惰性宽度定型**（comptime_int/comptime_float，使用处定型）；一切控制流、资源与错误路径显式可见（见 §3）。
...
语言不提供任何隐式控制流或隐式资源行为——无异常、无宏黑魔法、无隐式内存分配；资源释放显式（defer/errdefer）、所有权转移显式（owned/move）、错误路径显式（!T + try/catch）。隐式白名单仅两项：局部变量类型推断（仅 var 且可无歧义推断）与字面量惰性宽度定型。
_Avoid_: 异常, 宏, 隐式分配, 「一切都是隐式/显式」的绝对化表述（隐式白名单见 §0 规则 4）

**接口契约 (Interface as contract, 2026-08-31 定案措辞)**:
接口是编译期功能契约——**不能实例化，只能被类型显式实现**（`class Rect: IShape`）或**作为泛型约束**（`where T: IShape`，静态单态化为主）或**作为接口引用**（`*IShape` 胖指针，异构集合用，动态分发为辅）。接口可继承接口；具体类型（class/struct）不可继承。
_Avoid_: 接口实例化 / 把接口当 trait 宏或鸭子类型

**模块边界 (Module context boundary, ADR-0026)**:
模块 = `src/Modules/` 下的一个子目录，是一个封闭的可自我运行的功能单元：对内自组织，对外仅暴露 context 结构体与接口定义；模块内类型直接创建外部类型 = 编译错误；模块间依赖只有两条正交通道——`import`（符号引用/API 面）与 `context`（IoC 依赖注入，AppContext→ModuleContext 层级委托、背靠 Arena）。
_Avoid_: 直接 new 外部模块类型 / 把 import 当依赖注入
...
**所有权 (Ownership, 2026-08-29 修订：指针形态转移模型，ADR-0030)**:
所有权 = **销毁责任的唯一归属**——`owned` 标注的变量必须由 `defer` 显式释放或经 `move` 转移；**不是访问权限控制**。权限三要素正交：可读（默认）、可写（`mut`）、拥有（`owned`）。**转移载体 = 指针形态**：接收方参数声明 `owned *T`（只读拥有）/ `owned *mut T`（可写拥有）；调用点 `move &t` / `move &mut t`。值形态参数（`owned T`）不再用于转移。
...
| `T` | ✅ | ❌ | ❌ | 值类型或不可变引用 |
| `mut T` | ✅ | ✅ | ❌ | 允许修改，无所有权 |
| `owned T` | ✅ | ❌ | ✅ | 拥有所有权，必须 `defer` 或经 `move &t` 转移 |
| `owned mut T` | ✅ | ✅ | ✅ | 拥有所有权且可修改，可 `move &mut t` 转移 |

**规则**：
1. **作用域不自动释放**。凡堆创建的对象、文件句柄、数据连接等外部资源，统一用 `defer` 在退出作用域时显式释放。
2. **无 `defer` 则必须转移**。`owned` 变量必须匹配 `defer` 语句或将所有权经 `move &t` / `move &mut t` 转移，否则编译错误。
3. **栈/Arena 分配无所有权**。标量/Continuous 类型（栈上）以及 Arena 分配的对象，均无 `owned`，不需要 `defer`，由栈帧或 Arena 统一管理。**Arena 分配与 `global` 变量禁止作为 move 源**（内存由 Arena/全局对象管理，虽为堆分配）。
4. **`owned` 只能转移一次（affine type）**。**move 后原变量冻结**：后续读/写编译错误，重新赋值可复活（2026-08-29 修订：旧「原绑定仍可访问」废止——双持销毁责任正是所有权要防的）。
...
**move (2026-08-29 修订：指针形态 + 别名 + 返回推断，ADR-0030)**:
转移变量销毁责任的操作——不复制数据、不移动内存。**合法形态**：`move &t`（只读拥有转移）/ `move &mut t`（可写拥有转移）；**`move t` 是 `move &mut t` 的字面别名**（对非 `mut` 变量报错——改写 `move &t` 或加 mut）。**move 后原变量冻结**：后续使用编译错误。**返回推断**：签名返回 `T`，若函数体 `return` 堆分配构造（`alloc.init(...)`）或 `return &mut expr`，签名自动推断为 `owned *mut T`（用户仍只写 T）；裸值 return（标量/切片）不推断，保持值语义。`move \|x\|` 闭包捕获形态与所有权 move 无关（正交，保留）。
_Avoid_: 把 move 当作复制；move 后继续使用原变量

**可移动性 (Movability, 2026-08-29 修订：三条件)**：
move 的约束 = ① 变量标注 `owned`；② 分配来源 = 全局分配器（`alloc.init`）——**Arena 分配、global 变量禁止 move**（虽为堆分配，内存由 Arena/全局对象管理，ADR-0030）；③ 未移出过（affine）。指针别名自由（用户负责），不阻塞 move。
...
### 同命名空间共享（ADR-0031，2026-08-29）

**同目录文件 = 同命名空间 = 扁平互见**：`src/*.hc` 同级文件的顶层声明直接互相可见（无需 import），调用点用扁平名。**跨命名空间**（子目录/Modules）用 `import NS.{sym}` / 限定调用。同名声明冲突 → 编译错误（列两处来源），无静默覆盖。
_Avoid_: 靠目录扫描隐式合并后又称“需要 import”

### 包模式（Package Mode，stage1 interp）

stage1 interp（`hc run stage1/interp.hc <target>`）对入口文件名为 `main.hc` 的目标自动加载同目录全部 `.hc`（字典序合并，确定性）；单文件目标保持 `import .{sym}` 显式装载（stage1 interp 专有扩展，不入语言规范，详见 ADR-0031）。循环导入允许（幂等合并、每文件只展开一次）。
_Avoid_: 把 `import .{sym}` 当作语言规范语法
